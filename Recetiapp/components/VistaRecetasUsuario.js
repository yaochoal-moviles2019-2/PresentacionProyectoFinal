...
export default class VistaRecetasUsuario extends Component {
    ...
    componentDidMount() {
        axios.get(backurl + "recipes")
            .then(result => { this.setState({ recetas: result.data }) })
            .catch(error => { Alert.alert("Error Obteniendo recetas de Back"); console.log(error) })
    }

    VerReceta = (item) => {
        var params = { item: item }
        Actions.VistaRecetaUsuario(params);
    }
